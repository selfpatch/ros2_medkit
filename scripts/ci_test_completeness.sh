...
if [[ -z "${START_EPOCH}" || -z "${CAP_MINUTES}" ]]; then
...
# The runner kills at the cap, and this script starts a moment later, so treat
# anything within a minute of the cap as an overrun rather than demanding it be
# over exactly.
overrun_threshold=$(( cap_seconds - 60 ))
...
if [[ "${elapsed}" -ge "${overrun_threshold}" ]]; then
...
if [[ "${missing}" -gt 0 && "${elapsed}" -lt "${overrun_threshold}" ]]; then
