...
///
/// Round-trip note: for opaque ``nlohmann::json`` fields the canonical value
/// is an empty object (``{}``) rather than the default-constructed
/// ``nlohmann::json{}`` (which is JSON null). This matters when the field is
/// wrapped in ``std::optional<nlohmann::json>``: the reader treats JSON null
/// as "absent", so a default-constructed sample would lose the field on
/// round-trip even though the writer happily emitted it.
template <class U>
U sample_value() {
  if constexpr (is_optional_v<U>) {
    if constexpr (std::is_same_v<typename U::value_type, nlohmann::json>) {
      return nlohmann::json::object();  // present, round-trips as `{}`
    } else {
    ...
    return nlohmann::json::object();  // opaque object: round-trips as `{}`
