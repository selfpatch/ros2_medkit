...
class IAuthRequirementPolicy {
 public:
  ...
  /**
   * @brief Whether an operator named this exact route in `auth.public_routes`
   *
   * Narrower than `!requires_authentication(...)`, and the difference is the
   * point. Under `require_auth_for: "write"` every GET is answered without a
   * credential because of the requirement level, which is not the same
   * statement as "somebody took this route outside authentication on purpose".
   * A handler that withholds part of its answer from an uncredentialed caller
   * asks THIS question, so the withholding follows the operator's list and not
   * the requirement level.
   *
   * False for every policy but PublicRouteExemptionPolicy, which is the only
   * one that carries such a list.
   *
   * @param method HTTP method
   * @param path Request path
   * @return true if the route is exempted by name
   */
  virtual bool is_public(const std::string & method, const std::string & path) const {
    (void)method;
    (void)path;
    return false;
  }
...
/// True for an entry that is empty or whitespace only, as std::isspace reads
/// it - the same rule parse_public_route trims by.
///
/// `[""]` is how a ROS 2 YAML file writes an empty string sequence - the
/// shipped profiles use it for `auth.clients` and for the bulk-data
/// categories - so a list written that way carries one blank entry and means
/// "no routes". Callers skip such an entry; it is neither a route nor a typo.
bool is_blank_public_route_entry(const std::string & entry);

/// Parses a whole `auth.public_routes` list, skipping blank entries and
/// dropping entries that do not parse. Dropping keeps the route protected,
/// which is the safe reading of a malformed entry; GatewayNode validates the
/// list first and refuses to start so a typo cannot silently protect a route
/// the operator wanted open.
 ...
 * stricter than the policy underneath, and it cannot shadow it.
 *
 * Both shipped profiles leave the list empty, which makes this a no-op: a
 * route is open exactly where somebody said so and nowhere else.
 */
class PublicRouteExemptionPolicy : public IAuthRequirementPolicy {
 public:
  ...
  bool is_public(const std::string & method, const std::string & path) const override;
