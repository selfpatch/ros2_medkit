 ...
 */
struct TlsConfig {
  /// Whether TLS is enabled. False here is the value a default-constructed
  /// TlsConfig carries, not the posture the gateway ships: the shipped
  /// config/gateway_params.yaml sets server.tls.enabled true.
  bool enabled{false};
