...
A recording belongs to the fault records it is attached to, each a (`fault_code`, owner) pair. `GET /api/v1/{entity-path}/bulk-data/rosbags/{recording_id}` serves it when the owner of one of those records is in the entity's scope, which is exactly when the entity's rosbags listing carries it. Owning a record of the same code is not enough: two apps reporting one code each download only their own recording, cleared or not, and the component hosting both serves both.
