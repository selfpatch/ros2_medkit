...
TopicSampleResult NativeTopicSampler::sample_topic(const std::string & topic_name, double timeout_sec) {
  ...
  RCLCPP_DEBUG(node_->get_logger(), "sample_topic: topic='%s' type='%s' pubs=%zu subs=%zu",
              topic_name.c_str(), info->type.c_str(), info->publisher_count, info->subscriber_count);
  ...
  if (info->publisher_count == 0) {
    RCLCPP_DEBUG(node_->get_logger(), "sample_topic: Topic '%s' has no publishers, returning metadata only", topic_name.c_str());
    result.has_data = false;
    return result;
  }
  ...
  try {
    ...
    cmd << "ros2 topic echo " << ROS2CLIWrapper::escape_shell_arg(topic_name)
        << " --once --no-arr --timeout " << timeout_int << " 2>/dev/null";
    ...
    if (!pipe) {
      RCLCPP_WARN(node_->get_logger(), "sample_topic: Failed to execute CLI command for topic '%s'", topic_name.c_str());
      result.has_data = false;
      return result;
    }
    ...
    RCLCPP_DEBUG(node_->get_logger(), "sample_topic: CLI finished, exit_code=%d, output_len=%zu",
    ...
    if (exit_code == 0 && !output.empty() && output.find("WARNING") == std::string::npos) {
      ...
      RCLCPP_DEBUG(node_->get_logger(), "sample_topic: No data received from topic '%s' (exit_code=%d)", topic_name.c_str(), exit_code);
      result.has_data = false;
    }
...
std::map<std::string, ComponentTopics> NativeTopicSampler::build_component_topic_map() {
  ...
  for (const auto & [topic_name, types] : all_topics) {
    ...
    for (const auto & pub_info : publishers_info) {
      ...
      std::string component_fqn = (ns == "/" || ns.empty()) ? "/" + name : ns + "/" + name;
