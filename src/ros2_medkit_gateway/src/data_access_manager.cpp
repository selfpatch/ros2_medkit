...
json DataAccessManager::get_topic_sample_native(const std::string & topic_name, double timeout_sec) {
  RCLCPP_DEBUG(node_->get_logger(), "get_topic_sample_native: topic='%s', timeout=%.2f", topic_name.c_str(), timeout_sec);
  auto sample = native_sampler_->sample_topic(topic_name, timeout_sec);
  RCLCPP_DEBUG(node_->get_logger(), "get_topic_sample_native: sample returned, has_data=%d, type='%s'",
              sample.has_data, sample.message_type.c_str());
