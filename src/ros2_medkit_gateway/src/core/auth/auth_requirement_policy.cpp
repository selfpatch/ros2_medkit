...
// Whitespace as std::isspace reads it, so the rule that skips a blank entry and
// the rule that trims a real one agree on what blank means.
static std::string trim_whitespace(const std::string & entry) {
...
bool is_blank_public_route_entry(const std::string & entry) {
  return trim_whitespace(entry).empty();
}
...
tl::expected<PublicRoute, std::string> parse_public_route(const std::string & entry) {
  ...
  // Wildcards are refused, and never quietly ignored. Accepting the character and
...
bool PublicRouteExemptionPolicy::is_public(const std::string & method, const std::string & path) const {
