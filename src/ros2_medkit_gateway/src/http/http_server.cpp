...
void HttpServerManager::configure_tls() {
  ...
  // Set the protocol floor on our own context, so nothing is inherited.
  //
  ...
  if (ctx == nullptr || SSL_CTX_set_min_proto_version(ctx, min_proto) != 1) {
    // Refuse to serve. Falling back to the library floor would hand a caller
    // that asked for TLS 1.3 a connection negotiated at 1.1, and nothing
    // downstream can tell the two apart - so the weaker transport would look
    // exactly like the one that was configured.
