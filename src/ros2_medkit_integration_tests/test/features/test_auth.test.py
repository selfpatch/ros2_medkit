...
Launches the gateway on port 8085 with auth enabled (JWT HS256, four client
...
class TestAuthenticationIntegration(GatewayTestCase):
...
class TestAuthorizationIntegration(GatewayTestCase):
