...
Launches TWO gateway instances with different CORS configurations:
- Port 8085: CORS enabled, credentials disabled, two allowed origins
- Port 8086: CORS enabled, credentials enabled, one allowed origin
...
def generate_test_description():
    ...
    gateway_no_creds = create_gateway_node(
    ...
    gateway_with_creds = create_gateway_node(
...
class TestCorsIntegration(GatewayTestCase):
    """CORS integration tests.

    Uses custom setUpClass because this test waits for TWO gateway instances.
    Inherits from GatewayTestCase for the health-check helper.
    """
    ...
    # Override BASE_URL for GatewayTestCase health check (uses first gateway)
    BASE_URL = BASE_URL_NO_CREDS
