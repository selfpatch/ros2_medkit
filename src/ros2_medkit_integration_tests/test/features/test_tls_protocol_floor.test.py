...
the floor is shown to MOVE with the setting, which a single reading cannot
separate from it happening to sit where
...
def _handshake(port, version, client_cert=None, client_key=None, timeout=20):
    ...
    complete, so that is what is read here; the exit status says less.
    """
...
def _wait_listening(port, timeout=60.0):
    ...
    reason and the acceptance assertions fail at random, differently each run.
...
class TestTlsProtocolFloor(unittest.TestCase):
    ...
    def test_01b_the_client_actually_offers_the_old_versions(self):
        ...
        refusals in test_02 prove nothing, and this fails to say so.
        """
    ...
    def test_03_floor_13_refuses_12(self):
        ...
        A gateway configured for 1.3 that still completes a 1.2 handshake has
        read the value, logged it and ignored it - which is indistinguishable
        from a correct one unless somebody tries the lower version. TLS 1.2 is
        accepted by the OTHER gateway in this same launch, so a failure here
        cannot be blamed on the client or on the certificate.
        """
