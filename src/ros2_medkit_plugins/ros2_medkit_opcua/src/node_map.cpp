...
bool NodeMap::load(const std::string & yaml_path) {
  try {
    ...
    // Warn on keys a loader block does not recognise so a future typo (e.g.
    // ``severty:``) is surfaced instead of silently dropped. Shared by the
    // ``auto_browse:`` map form below, the event_alarms loader, and the
    // auto_alarms loader.
    ...
    // A config that declares none of 'nodes:', event alarms, auto_browse, nor
    // auto_alarms carries no mappings and is almost always a mistake (wrong
    // file / typo). A present-but-empty 'nodes:' section is still valid, and
    // an auto_browse-only or auto_alarms-only file (zero-config discovery:
    // the tree / alarms are filled at runtime) is valid too.
    if (!has_nodes && event_alarms_.empty() && !auto_browse_config_.enabled && !auto_alarms_.enabled) {
      RCLCPP_ERROR(rclcpp::get_logger("opcua.node_map"),
                   "Node map has neither 'nodes:', 'event_alarms:', 'auto_browse:' nor 'auto_alarms:' entries - "
                   "nothing to map");
      entity_defs_.clear();
      return false;
    }
