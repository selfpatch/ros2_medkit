...
bool NodeMap::load(const std::string & yaml_path) {
  try {
    ...
    // event_alarms and auto_alarms loaders below.
    ...
    // nor auto_alarms carries no mappings and is almost always a mistake
    // (wrong file / typo). A present-but-empty 'nodes:' section is still a
    // valid config.
    if (!has_nodes && event_alarms_.empty() && !auto_alarms_.enabled) {
      RCLCPP_ERROR(rclcpp::get_logger("opcua.node_map"),
                   "Node map has neither 'nodes:', 'event_alarms:' nor 'auto_alarms:' entries - nothing to map");
      entity_defs_.clear();
      return false;
    }
