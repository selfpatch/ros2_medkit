...
bool NodeMap::load(const std::string & yaml_path) {
  try {
    ...
    // the ``auto_browse:`` map form below and the ``event_alarms:`` /
    // ``auto_alarms:`` loaders.
    ...
    // still valid, and an auto_browse-only / auto_alarms-only file
    // (zero-config discovery: the tree and/or alarms are filled at runtime)
    // is valid too.
