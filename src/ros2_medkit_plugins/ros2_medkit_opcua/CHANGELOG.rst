...
* ``PLC_COMMS_LOST`` is reported under the PLC runtime component, which holds no data values of its own, so its fault detail used to carry no freeze-frame at all. It is now framed from the apps that component hosts: one entry per hosted app, named after the app, and none for the component. The entries carry ``connected`` and ``source_timestamp`` only when the plugin's payload reports them, which the ``DataProvider`` path (``list_data``) does not do today, so a frame taken through it names ``source`` and ``captured_at`` and nothing further
* Contributors: @bburda
