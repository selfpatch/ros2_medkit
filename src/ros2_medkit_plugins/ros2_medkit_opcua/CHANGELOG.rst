...
* The OpenPLC tank demo (``docker/scripts/start.sh``) now runs a ``fault_manager_node`` beside the gateway, so an alarm the PLC raises becomes a fault the SOVD API serves instead of a 503 from every ``/faults`` route. Start-up waits for ``/fault_manager/report_fault`` to be advertised and fails with a clear message when it never appears, and ``faults.db`` is written to the same named volume as the entity freeze frames, so a fault and the frame it carries both outlive ``scripts/stop.sh``
* Contributors: @bburda
